        ...
        <q-dialog v-model="alert" >
            <q-card class="bg-color" >
                ...
                <q-card-section class="q-pt-none" v-model="seleciona_placas_video">
                    ...
                    v-for="caracteristica in placas_video.placas_video" :key="caracteristica.modelo" :value="caracteristica.modelo">
                        <div class="col-12 ">
                        ...
                        <div class="col-12 text-center q-pt-xl">
                            <q-btn label="Incluir" color="primary"  @click="seleciona_placas_video = caracteristica.modelo, enviarPlacaVideo()" v-close-popup/>
                        </div>
                    </div>
                </q-card-section>
...
export default defineComponent({
    ...
    created() {
        axios.get('/DB/db.json')
            .then(response => {
                this.placas_video = response.data;
            })
            .catch(error => {
    ...
    methods:{
        enviarPlacaVideo(){
            this.$emit('placaVideo',
                this.seleciona_placas_video
            )
        }
    }
})
